    ...
    fn render_inner(&mut self, state: &mut DebuggerState, ui: &Ui) {
        ...
        if let Some(elf_bin) = &state.elf {
            if let BinaryFile::Elf(elf_buffer) = elf_bin {
                ...
                let subprograms: Vec<SubProgram> = debugging_info::parse_dwarf_info(&elf_buffer, &elf_parsed).subprograms;
                ...
                if let Some(Some(user_regs)) = &state.process_state.first().map(|p| p.cache_user_regs.as_ref()) {
                    ...
                    for section in elf_parsed.program_headers {
                        ...
                        section_data[..section.p_filesz as usize].copy_from_slice(elf_buffer.get((section.p_offset as usize)..(section.p_offset as usize + section.p_filesz as usize)).unwrap());

                        //TODO: get name
                        sections.push((section.p_vaddr, section_data, format!("{:X}", section.p_vaddr)));
                    }
                    ...
                    for (virt_addr, section_data, name) in &sections {
                        ...
                        while decoder.can_decode() {
                            ...
                            if let Some((_, name)) = section_names.iter().find(|(addr, _)| *addr == instruction.ip() - load_address) {
                                ui.text(format!("{}:", name));
                            }
