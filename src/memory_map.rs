...
use imgui::sys::{igBeginTable, igEndTable, igNextColumn, igTableNextColumn, igText, ImGuiTableFlags, ImVec2};
use libc::stat;
    ...
    fn render_inner(&mut self, state: &mut DebuggerState, ui: &Ui) {
        if let Some(proc) = state.process {
            if let Some(mmap) = ptrace::get_memory_map(proc.0) {
                ImGuiTableBuilder::with_name(CString::new("mmap").unwrap(), 5, |s| {
                    ...
                    for entry in &mmap.0 {
                        ...
                        b.next_column();
                        ui.text(format!("{}{}{}", if entry.permissions.read { "R" } else { "-" }, if entry.permissions.write { "W" } else { "-" }, if entry.permissions.execute { "X" } else { "-" }));
                        b.next_row();
                    }
                });
            } else {
