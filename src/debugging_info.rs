    ...
    if let Ok(dwarf) = dwarf {
        ...
        while let Some(header) = iter.next().unwrap() {
            ...
            while let Some((delta_depth, entry)) = entries.next_dfs().unwrap() {
                ...
                let attrs = entry.attrs().iterator().map(|a| a.unwrap()).collect::<Vec<_>>();

                match entry.tag() {
                    gimli::DW_TAG_subprogram => {
                        let name = {
                            let name = attrs.iter().find(|a| a.name() == gimli::DW_AT_name).expect("No name");
                            let name_str = dwarf.attr_string(&unit, name.value()).expect("Failed to get name string");
                            use gimli::Reader;
                        ...
                        let start_addr = {
                            let pc = attrs.iter().find(|a| a.name() == gimli::DW_AT_low_pc).map(|pc|
                                match pc.value() {
